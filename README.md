...
After `v2.2.0` we've add server render supporting in byted-react-scripts.

> It is suggested that if you use SSR, a state container (for example, [redux](https://github.com/reduxjs/redux) | [mobx](https://github.com/mobxjs/mobx) | [rematch](https://github.com/rematch/rematch)) which can run both in browser and node is required. We need to recover the server side react component state in client after page is rendered.

__SSR feature required:__

1. a `ssr.js` or `ssr.tsx` (depends on the `DEV_LANG` field in `.env` file whether is `typescript`).
2. for a `typescript` project, make sure there's a tsconfig.ssr.json in root path.

__Migrate your code for SSR:__

We provide a `process.env.REACT_APP_IN_NODE` global variable which equals `String(true)` in code context.
...
1. react-router
   ...
   const history = process.env.REACT_APP_IN_NODE === 'true'
   	? createMemoryHistory()
   ...
   const Router = process.env.REACT_APP_IN_NODE === 'true'
   	? StaticRouter
   	: BrowserHistory;
...
2. index.js
   ...
   // _INIT_STATE_ represents the whole app current state from server side
     window._store_.dispatch({
     // dispatch an Reset action to sync the client app state
       type: 'RESET',
       payload: window._INIT_STATE_
     });
     ReactDOM.hydrate(
...
3. ssr.js
    ...
    * @param {empty object} context
    *
    * see:https://reacttraining.com/react-router/web/guides/server-rendering
    */
   export function render(store, location, context) {
   ...
   // server side code
   // ...
   const { render } = require('bundle.ssr.js');
   const store = createStore();
   const data = await ctx.service.getPageData();	// get the entry page data
   store.dispatch({	// sync the page data into store
       type: 'xxx',
       payload: data
   });
   const routerContext = {};
   const pageString = render(store, req.pathname, routerContext);
   if (routerContext.url) {
       ctx.redirect(routerContext.url);
   } else {
       ctx.render('template.html', { pageString, _INIT_STATE_: store.getState() });
   }
   // ...
   ```
